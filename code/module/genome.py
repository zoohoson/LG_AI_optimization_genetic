...
class Genome():
    ...
    def forward(self, inputs):
        ...
        net += 1  # make 1~2
        net = net * self.mask  # mask False인 곳은 0으로
        out1 = self.event_map[np.argmax(net)]  # out1은 이벤트 출력
        ...
        net = self.softmax(net)
        out2 = np.argmax(net)
        out2 /= 2  # 0 ~ 5.5, step 0.5이면 12개의 output, 시간별 최대개수인 6.667개 이하를 만족하려고 2로 나눔
        return out1, out2
    ...
    # input_length에서 슬라이딩하는 만큼 order파일아래에 0값 추가
    def create_order(self, order):
        for i in range(30):
            order.loc[91+i,:] = ['0000-00-00', 0, 0, 0, 0]
        return order
   
   # order파일을 보고 필요한 EVENT와 MOL 개수를 predict
